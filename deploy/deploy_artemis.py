...
class repo:
    ...
    def deploy(self):
        ...
        os.chdir(self.deploy_location)
        print(f"Setting up environment in {self.deploy_location}")
    ...
    """
        ...
        if move_symlink == "y":
            live_location = os.path.join(release_area, self.name)
            new_tmp_location = os.path.join(release_area, "tmp_art")
            os.symlink(self.deploy_location, new_tmp_location)
...
def get_artemis_release_dir_from_args(repo: repo) -> str:
    ...
    if args.beamline == "dev":
        ...
        return f"/dls_sw/{args.beamline}/software/bluesky/artemis_"
