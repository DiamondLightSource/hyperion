...
def mock_ispyb_get_time():
    with patch(
        "artemis.external_interaction.callbacks.fgs.ispyb_callback.Store3DGridscanInIspyb.get_current_time_string"
    ) as p:
        yield p
...
def mock_ispyb_store_grid_scan():
    with patch(
        "artemis.external_interaction.callbacks.fgs.ispyb_callback.Store3DGridscanInIspyb.store_grid_scan"
    ) as p:
        yield p
...
def mock_ispyb_begin_deposition():
    with patch(
        "artemis.external_interaction.callbacks.fgs.ispyb_callback.Store3DGridscanInIspyb.begin_deposition"
    ) as p:
        yield p
...
def mock_ispyb_end_deposition():
    with patch(
        "artemis.external_interaction.callbacks.fgs.ispyb_callback.Store3DGridscanInIspyb.end_deposition"
    ) as p:
        yield p
