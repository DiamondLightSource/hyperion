...
class DetectorDistanceToBeamXYConverter:
	...
	def get_beam_xy_from_det_dist_mm(self, det_dist_mm: float, beam_axis_values: list) -> float:
		det_dist_array = self.lookup_table_values[0]
		return interp(det_dist_mm, det_dist_array, beam_axis_values)

	def get_beam_axis_pixels(self, det_distance: float, image_size_pixels: int, det_dim: float, beam_axis_values: list) -> float:
