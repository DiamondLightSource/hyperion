...
def test_microns_to_xyz_move_returns_the_same_values_as_GDA(
    h, v, omega, expected_values, mock_centring: OAVCentring
):
    results = np.around(mock_centring.microns_to_xyz_move(h, v, omega), decimals=2)
...
def test_get_formatted_edge_waveforms(mock_centring: OAVCentring):
    ...
    recieved_top, recieved_bottom = tuple(mock_centring.get_formatted_edge_waveforms())
