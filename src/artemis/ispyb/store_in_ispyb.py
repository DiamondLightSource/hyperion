...
def store_grid_scan(grid_info: GridInfo, data_collection: DataCollection, data_collection_group: DataCollectionGroup,
                    position: Position):

    with ispyb.open(ISPYB_CONFIG_FILE) as conn:
        group_id = store_data_collection_group_table(data_collection_group, conn)
        position_id = store_position_table(position, conn)

        data_collection.position_id = position_id
        data_collection.data_collection_group_id = group_id

        dc_id = store_data_collection_table(data_collection, conn)

        grid_info.ispyb_data_collection_id = dc_id
        grid_id = store_grid_info_table(grid_info, conn)
...
def store_data_collection_table(data_collection: DataCollection, conn: Connector) -> int:
    core = conn.core
    mx_acquisition = conn.mx_acquisition
...
def store_position_table(position: Position, conn: Connector) -> int:
    mx_acquisition = conn.mx_acquisition
...
def store_data_collection_group_table(data_collection_group: DataCollectionGroup, conn: Connector) -> int:
    core = conn.core
    mx_acquisition = conn.mx_acquisition
