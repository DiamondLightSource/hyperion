...
def test_store_grid_scan(ispyb_conn, dummy_ispyb):
    ...
    when(dummy_ispyb)._store_data_collection_group_table().thenReturn(TEST_DATA_COLLECTION_GROUP_ID)
    when(dummy_ispyb)._store_data_collection_table(TEST_POSITION_ID, TEST_DATA_COLLECTION_GROUP_ID).thenReturn(TEST_DATA_COLLECTION_ID)
    when(dummy_ispyb)._store_grid_info_table(TEST_DATA_COLLECTION_ID).thenReturn(TEST_GRID_INFO_ID)
...
def test_param_keys(ispyb_conn, dummy_ispyb):
    ...
    when(ispyb_conn.return_value.mx_acquisition).get_dc_position_params().thenReturn(POSITION_PARAMS)

    when(ispyb_conn.return_value.core).retrieve_visit_id(ANY).thenReturn(TEST_SESSION_ID)
    when(ispyb_conn.return_value.mx_acquisition).upsert_data_collection(ANY).thenReturn(TEST_DATA_COLLECTION_ID)
    when(ispyb_conn.return_value.mx_acquisition).update_dc_position(ANY).thenReturn(TEST_POSITION_ID)
    when(ispyb_conn.return_value.mx_acquisition).upsert_data_collection_group(ANY).thenReturn(TEST_DATA_COLLECTION_GROUP_ID)
    when(ispyb_conn.return_value.mx_acquisition).upsert_dc_grid(ANY).thenReturn(TEST_GRID_INFO_ID)
