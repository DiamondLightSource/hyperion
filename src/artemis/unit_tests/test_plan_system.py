...
def test_getting_data_for_ispyb():
    ...
    s4slits = S4Slits(f"{SIM_BEAMLINE}-AL-SLITS-04:", name="s4slits")

    undulator.wait_for_connection()
    synchrotron.wait_for_connection()
    s4slits.wait_for_connection()

    RE = RunEngine()

    @bpp.run_decorator()
    def standalone_read_hardware_for_ispyb(und, syn, s4slits):
        yield from read_hardware_for_ispyb(und, syn, s4slits)
