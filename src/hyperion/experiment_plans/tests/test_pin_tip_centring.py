...
async def test_given_tip_at_zero_but_will_be_found_when_get_tip_into_view_then_smargon_moved_negative_and_tip_returned(
    ...
    set_sim_value(smargon.x.low_limit_travel, -2)
    set_sim_value(smargon.x.high_limit_travel, 2)
    ...
    smargon.x.setpoint.subscribe(set_pin_tip_when_x_moved)

    result = RE(move_pin_into_view(oav, smargon))

    assert await smargon.x.setpoint.get_value() == -DEFAULT_STEP_SIZE
    assert result.plan_result == (100, 200)
...
async def test_pin_tip_starting_near_negative_edge_doesnt_exceed_limit(
    ...
    set_sim_value(smargon.x.readback, -1.8)
    oav.mxsc.pin_tip.tip_x.sim_put(0)
    oav.mxsc.pin_tip.tip_y.sim_put(100)
    ...
    assert await smargon.x.setpoint.get_value() == -2
...
def test_given_moving_out_of_range_when_move_with_warn_called_then_warning_exception(
    ...
    set_sim_value(smargon.x.high_limit_travel, 10)
...
async def test_when_pin_tip_centre_plan_called_then_expected_plans_called(
    ...
    set_sim_value(smargon.omega.readback, 0)
    ...
    assert await smargon.omega.setpoint.get_value() == 90
